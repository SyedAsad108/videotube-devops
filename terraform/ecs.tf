...
# ------------------------------------------
# Frontend ECS Task Definition (EC2 mode)
# ------------------------------------------
  ...
  container_definitions = jsonencode([
    {
      ...
      healthCheck = {
        command     = ["CMD-SHELL", "wget -q --spider http://localhost:80/healthz || exit 1"]
        interval    = 30
        timeout     = 5
        retries     = 3
        startPeriod = 15
...
# ------------------------------------------
# Frontend ECS Service (EC2 capacity provider strategy)
# ------------------------------------------
